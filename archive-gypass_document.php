...
<section id="hero" class="hero-listing">
    <div class="container">
        <h1 class="h2-size"><?php echo get_field('documents_title', 'options') ?: __("Documents", "gypass"); ?></h1>
        ...
        if (!empty($document_types) && is_array($document_types)) : ?>
            <div class="filters">
                ...
                <a href="<?php echo esc_url(get_post_type_archive_link('gypass_document')) ?>" class="btn btn-outline-primary active">
                    <?php echo __("Tout", "gypass"); ?>
                </a>
                <?php foreach ($document_types as $document_type) : ?>
                    <a href="<?php echo esc_url(get_term_link($document_type)) ?>" class="btn btn-outline-primary">
